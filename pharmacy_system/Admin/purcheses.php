<?php
    ...
    if(isset($_SESSION["id"])) {
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $st3 = $con->prepare("UPDATE auto_bill_number SET bill_no = bill_no + 1 WHERE bill_type = 2");
            $st3->execute();

            // Adding qunatity of items of bill quantity
            ...
            // Add bill to DB
            $stmt = $con->prepare("INSERT INTO purchases_bill (`bill_no`, `bill_date`, `quantity`, `pur_price`, `discount`, `net`, `bill_type`, `item_id`, `sup_id`) VALUES (?,now(), ?, ?, ?, ?, 2, ?, ?)");
            $stmt->execute(array(
                $bill_no,
                $quantity,
                $price,
            ...
            if($result) {
                echo '
                    <div class="alert alert-success">Insert To Purcheses Bill Success</div>
                ';
                header("refresh:2; url=index.php?page-title=items-status");
            } else {
                echo '
                    <div class="alert alert-danger">Insert To Purcheses Bill Failed</div>
                ';
                header("refresh:2; url=sales_bill.php");
            }
            // add Bill to Sales & Return Movement
            $stmt4 = $con->prepare("INSERT INTO `sale_return_move`(`date`, `bill_type`, `sup_id`, `client_name`, `total_money`) VALUES (now(), 2, ?, ?, ?)");
            $stmt4->execute(array(
                $sup_id,
                "NULL",
                $total
...
?>
<div class="container">
    <h2 class="text-center">Purcheses Bill</h2>
            ...
            <input type="text" name="bill-no" value="
                <?php 
                    ...
                    foreach($rows as $row) {
                        if($row["bill_type"] == "2") {
                            if($row["year"] != date("Y")) {
                                $st2 = $con->prepare("UPDATE auto_bill_number SET `year`=? WHERE bill_type = 2");
                                $st2->execute(array(date("Y")));
                                $st3 = $con->prepare("UPDATE auto_bill_number SET bill_no = 0 WHERE bill_type = 2");
                                $st3->execute();
                                $row["bill_no"] = 0;
                            }
                            if($row["bill_no"] == 0) {
