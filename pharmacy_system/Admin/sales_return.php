<?php
    ...
    if(isset($_SESSION["id"])) {
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $st3 = $con->prepare("UPDATE auto_bill_number SET bill_no = bill_no + 1 WHERE bill_type = 1");
            $st3->execute();
            ...
            // Add bill to DB
            $stmt = $con->prepare("INSERT INTO sales_bill (`bill_no`, `bill_date`, `quantity`, `sale_price`, `discount`, `net`, `bill_type`, `item_id`, `client_name`) VALUES (?,now(), ?, ?, ?, ?, 1, ?, ?)");
            $stmt->execute(array(
                $bill_no,
                $quantity,
                $price,
            ...
            $stmt4 = $con->prepare("INSERT INTO `sale_return_move`(`date`, `bill_type`, `sup_id`, `client_name`, `total_money`) VALUES (now(), 1, ?, ?, ?)");
            $stmt4->execute(array(
                $sup_id,
                $name,
                $total
...
?>
<div class="container">
    <h2 class="text-center">Sales Return Bill</h2>
            ...
            <input type="text" name="bill-no" value="
                <?php 
                    ...
                    foreach($rows as $row) {
                        if($row["bill_type"] == "1") {
                            if($row["year"] != date("Y")) {
                                $st2 = $con->prepare("UPDATE auto_bill_number SET `year`=? WHERE bill_type = 1");
                                $st2->execute(array(date("Y")));
                                $st3 = $con->prepare("UPDATE auto_bill_number SET bill_no = 0 WHERE bill_type = 1");
                                $st3->execute();
                                $row["bill_no"] = 0;
                            }
                            if($row["bill_no"] == 0) {
